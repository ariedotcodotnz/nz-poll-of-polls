...
tpm_sweep = site.electorate_sweep_risk("Te Pāti Māori")
if tpm_sweep is None:
    sweep_example = "The Te Pāti Māori electorate comparison is unavailable for this forecast."
else:
    tpm_none, tpm_none_independent = tpm_sweep
    sweep_example = (f"With the current assumptions that leaves Te Pāti Māori a {tpm_none} chance of holding "
                     f"none of its configured electorates, against {tpm_none_independent} if each seat were "
                     "an independent coin.")
...
1. **Electorates.** Each electorate in `config/electorates.yml` is won by the named party, an independent, or
   ...
   seats move together. The intercept is calibrated to preserve the configured probabilities at `at_share`
   after averaging over that shared shift. `{python} sweep_example`
